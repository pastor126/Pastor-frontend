...
const LoginForm: React.FC<LoginFormProps> = ({ setToken }) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    localStorage.clear(); // Limpa o localStorage antes de um novo login
    try {
  ...
  const links = [
    { name: 'Cadastro', href: '/novoUsuario' },
  ];
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      <div className="flex-1 border-4 border-black rounded-lg bg-gray-50 px-6 pb-4 pt-8">
        ...
        <div className="w-full">
          ...
          <div className="flex h-8 space-x-1 text-blue-500 hover:underline">
            {links.map((link) => (
              <Link
                key={link.name}
                href={link.href}
